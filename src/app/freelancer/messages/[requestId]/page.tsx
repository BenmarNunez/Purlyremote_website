...
import type { Message, HireRequest, ClientProfile } from '@/lib/supabase/types'

interface PageProps {
  params: Promise<{ requestId: string }>
}

export default function FreelancerChatPage({ params }: PageProps) {
    ...
    async function init() {
      ...
      if (reqData) {
        const { data: profile } = await supabase
          ...
          .eq('user_id', reqData.client_id)
          .single<Pick<ClientProfile, 'full_name' | 'company_name'>>()
        const display = profile?.full_name ?? 'Client'
        const withCompany =
          profile?.company_name ? `${display} · ${profile.company_name}` : display
        setClientName(withCompany)
      }
  ...
  const sendMessage = useCallback(async () => {
    ...
    const tempMsg: Message = {
      id: `temp-${Date.now()}`,
      sender_id: userId,
      receiver_id: hireRequest.client_id,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-neutral-200 px-4 py-3 flex items-center gap-3">
        <Link href="/freelancer/messages" className="text-neutral-400 hover:text-neutral-600 transition-colors" aria-label="Back to messages">
